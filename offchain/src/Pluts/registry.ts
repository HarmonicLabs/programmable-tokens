import { Address, Hash28, PrivateKey, Value } from "@harmoniclabs/plu-ts";
...
export async function mintFirstRegistryBuilder(blockfrost: BlockfrostPluts): Promise<string> {
  ...
  const aTokenPolicy = new Hash28(aToken.hash)
  ...
  const unsignedTx = txBuilder.buildSync({
    ...
    outputs: [{
      ...
      value: mintedValue,
      datum: registryDatum(aToken.hash, '', validators.transfer.hash, validators.userState.hash, validators.global.hash),
    },
      // {
