...
export async function allSendToAll() {
  ...
  console.log(`CRegistry UTxO: ${cRegistryUtxo[0].txHash}`)
  ...
  console.log(`User1State UTxO: ${cUser1StateUtxo[0].txHash}`)
  //  console.log(cUser1StateUtxo)
  const cUser2StateUtxo = await lucid.utxosAtWithUnit(cUserAddress, cUser2State)
  console.log(`User2State UTxO: ${cUser2StateUtxo[0].txHash}`)
  ...
  const cTransferAction = Data.to(new Constr(0, [[BigInt(9)]]))
  const aWithdrawRedeemer = Data.to(BigInt(10))
  const bWithdrawRedeemer = Data.to(BigInt(0))
  const cWithdrawRedeemer = Data.to(BigInt(9))
